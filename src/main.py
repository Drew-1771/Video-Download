...
def createDownload(window: tkinter.Tk, tab: tkinter.ttk.Frame, path: str, color: str) -> None:
    '''
    ...
    status.config(bg = color, fg="white")
...
def run():
    ...
    download_tab = tkinter.ttk.Frame(tabs)
    settings_tab = tkinter.ttk.Frame(tabs)
    tabs.add(download_tab, text="Download")
    tabs.add(settings_tab, text="Settings")
    tabs.pack(expand = 1, fill ="both")

    # populate tabs
    createDownload(window, download_tab, path, "gray9")
